...
{
    std::vector<Word>
        temp_arr; //I create a temporary vector to store all the stored Words and set those indices to an empty default Word Constructor
...
void WordTable::insert(Word jojo, int option)
{
    ...
    while (arr[(index + n * n) % capacity].word_name != ""
           && arr[(index + n * n) % capacity].word_name != jojo.word_name) {
        unsigned int multiple = n * n;
        Word &in_array
            = arr[(index + multiple)
                  % capacity]; //if the hash table is empty, each Word in the array is set using the default constructor, it is NOT NULL

        n += 1;
    }
    if (arr[(index + (n * n)) % capacity].word_name
        == "") //if the word at the index has word name "", that means it hasn't been assigned yet and can be used
    {
    ...
    } else if (arr[(index + (n * n)) % capacity].word_name
               == jojo.word_name) //if the word already exists, we need to add it's emotion tag
    {
...
Word WordTable::search(const std::string &text, int option)
{
    ...
    int n = 0;
    while (arr[(index + n * n) % capacity].word_name != ""
           && arr[(index + n * n) % capacity].word_name != text) {
        unsigned int multiple = n * n;
        Word &in_array
            = arr[(index + multiple)
                  % capacity]; //if the hash table is empty, each Word in the array is set using the default constructor, it is NOT NULL
        ...
        == "") //if the word at the index has word name "", that means it hasn't been assigned yet and can be used
    {
        return Word();
               ...
               == text) //if the word already exists, we need to add it's emotion tag
    {
...
std::vector<Word *> WordTable::emotion_analysis(int emotiontag)
{
    ...
    std::priority_queue<combinedWord, std::vector<combinedWord>, compareScores>
        pq; //this uses a special struct and bool to add Word Objects and their score and then sort them
    for (Word &a : arr) {
        if (a.word_name != "") {
    ...
    std::vector<Word *>
        finalarr; //keep in mind this returns a vector of POINTERS, you will need to use ->
    for (int i = 0; i < 10; i++) {
...
std::vector<float> WordTable::passage_sentiment_analysis(const std::string &passage, int option)
{
    ...
    while (std::getline(iss, word, ' ')) {
        ...
        for (int i = 0; i < 6; i++) {
            if (temp.word_name == "i" || temp.word_name == "the" || temp.word_name == "to"
                || temp.word_name == "in" || temp.word_name == "and" || temp.word_name == "feel"
                || temp.word_name == "and" || temp.word_name == "a" || temp.word_name == "of") {
